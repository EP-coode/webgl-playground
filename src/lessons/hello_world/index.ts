...
export function helloWorld(canvasSelector: string) {
  // 1. Canvas setup
  ...
  if (!colorLocation) {
    throw Error("Missing u_color variable in GLSL code. ");
  }

  drawRandomRectangles(canvas, gl, colorLocation, 15);
}

function drawRandomRectangles(
  canvas: HTMLCanvasElement,
  gl: WebGL2RenderingContext,
  rectColorLocation: WebGLUniformLocation,
  rectangeNumber: number
) {
  for (var i = 0; i < rectangeNumber; ++i) {
